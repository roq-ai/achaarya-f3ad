...
import { StudySessionListPage as ShowCaseList } from 'pages/study-sessions';
...
function DashboardPage() {
  ...
  const chapterStat = useSWR<PaginatedInterface<ChapterInterface>>(
    () => `/chapters`,
    () => getChapters({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Chapters',
      result: chapterStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`study-sessions`} passHref legacyBehavior>
                  <Button
